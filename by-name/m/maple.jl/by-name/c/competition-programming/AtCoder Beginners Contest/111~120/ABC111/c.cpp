...
signed main() {
  RAPID;
  // デフォルト変数定義
  int n=0,m=0,a=0,b=INF,c=0,d=0,x=0,y=0,z=0;
  string s="",t="";
  //
  ...
  for(auto p:Odd){
    mOdd.pb(p.second);
    x = max(x,p.second);
    if(x==p.second) a = p.first;
  }
  for(auto p:Even){
    mEven.pb(p.second);
    y = max(y,p.second);
    if(y==p.second) b = p.first;
  }
  RSORT(mOdd);
  RSORT(mEven);
  if(a!=b) OUT(n/2-mOdd[0] + n/2-mEven[0]);
  else OUT(min(n/2-mOdd[0]+n/2-mEven[1],n/2-mOdd[1]+n/2-mEven[0]));
}