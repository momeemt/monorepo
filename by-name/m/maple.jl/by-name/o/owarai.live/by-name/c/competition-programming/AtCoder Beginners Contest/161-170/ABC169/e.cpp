...
signed main() {
  ...
  vector<pair<double,int>> p1(n);
  vector<pair<double,int>> p2(n);
  REP(i,n) {
    p1[i].first = A[i];
    p1[i].second = i;
  }
  ...
  RSORT(p2);
  if (n%2==1) {
    int i1 = p1[n/2].second;
    int i2 = p2[n/2].second;
    out((int)(B[i2]-A[i1]+1));
  } else {
    int i1 = p1[n/2-1].second;
    int i2 = p1[n/2].second;
    int i3 = p2[n/2-1].second;
    int i4 = p2[n/2].second;
    int first = min(A[i1],A[i2]);
    int second = max(B[i3],B[i4]);
    out((int)(second-first+1)*2-3);
  }
  //   point[i].first = (B[i]+A[i])/2.0;
  //   point[i].second = i;
  // }
  // SORT(point);
  // if(n%2==1) {
  //   int idx = point[n/2].second;
  //   out((int)(B[idx]-A[idx]+1));
  // } else {
  //   int first = min(A[point[n/2-1].second],A[point[n/2].second]);
  //   int second = max(B[point[n/2-1].second],B[point[n/2].second]);
  //   out((int)(second-first+1)*2-3);
  // }
}