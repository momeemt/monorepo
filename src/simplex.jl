...
function to_canonical_form_lp(lp::LP)::LP
    ...
    for constraint in result_constraints
        if constraint.cmp == eq
            ...
            constraint2 = Constraint(Dict(k => -v for (k, v) in constraint.left), -constraint.right, le)
            append!(result.constraints, [constraint1, constraint2])
        elseif constraint.cmp == ge
            new_constraint = Constraint(Dict(k => -v for (k, v) in constraint.left), -constraint.right, le)
...
println(to_canonical_form_lp(lp))
