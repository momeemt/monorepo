...
また、細かく調整した設定について、Wordが勝手に変更を加えることもあります。もっとも頻発したのは改行時に発生するインデントのズレです。改行時に発生するため少し執筆を行うだけでも多発する上、設定の数値上は変更されていないので修正が厄介です。他にも、バグとしか思えない様な挙動や、再現性のない体裁崩れ、PDF出力時の体裁の変化など、Wordの問題は尽きません。
