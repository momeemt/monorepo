...
例えば、Wordでは数多くの設定を細かく調整することで、文章の体裁を整えることができますが、その分どの設定が何を意味しているのかを正確に把握することが大変難しいです。時には、設定同士が競合し、体裁が破壊されることもあります。
...
```
これを実現するには、`std.list`が`std.list-elem`コマンドのみを引数に持つ様な制約をプラグインが設定し、それをBrackが把握する必要があります。
...
Brackにおいては、プラグインのメタデータの一部として、コマンドの引数についてのアノテーションの情報を与えることで、プラグイン開発者が容易にこのアノテーションを設定することが可能です。先ほどの例で言えば、`std.list`の引数に対して`list-elem`の様なアノテーションを表示すれば、執筆者は入力するべきコマンドを把握しやすくなります。
