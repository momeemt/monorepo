...
signed main() {
  ...
  REP(i,h){
    REP(j,w){
      if(maze[i][j]=='#'){
        bool tmp1=true,tmp2=true,tmp3=true,tmp4=true;
        if(i != h-1 && maze[i+1][j]=='.') tmp1=false;
        if(i != 0 && maze[i-1][j]=='.') tmp3=false;
        if(j != w-1 && maze[i][j+1]=='.') tmp2=false;
        if(j != 0 && maze[i][j-1]=='.') tmp4=false;
        if(!tmp1 && !tmp2 && !tmp3 && !tmp4) ok = false;
      }
    }
  }
  Yes(ok);
