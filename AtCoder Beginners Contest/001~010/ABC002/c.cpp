...
signed main() {
  ...
  IN2(a,b);
  IN2(x,y);
  double side1 = sqrt(pow(n-a,2)+pow(m-b,2));
  double side2 = sqrt(pow(a-x,2)+pow(b-y,2));
  double side3 = sqrt(pow(x-n,2)+pow(y-m,2));
  double tmp = (side1+side2+side3)/2;
  FIX;
  OUT(sqrt(tmp*(tmp-side1)*(tmp-side2)*(tmp-side3)));
}