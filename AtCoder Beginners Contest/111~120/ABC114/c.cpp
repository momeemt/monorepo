...
void dfs(int rst, string s){
  if(rst == 0){
    pre753.pb(s);
    return;
  } else {
    for(int i = 0; i < 3; ++i){
      dfs(rst-1, s+components[i]);
    }
  }
}
...
signed main() {
  ...
  REP(i,pre753.size()){
    int tag = stoll(pre753[i]);
    if(tag <= N) {
      bool q7=false,q5=false,q3=false;
      REP(j, pre753[i].size()){
        if(pre753[i][j]=='7') q7=true;
        if(pre753[i][j]=='5') q5=true;
        if(pre753[i][j]=='3') q3=true;
      }
