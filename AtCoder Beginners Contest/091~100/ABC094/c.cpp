...
int bigN(Vi V, int key){RSORT(V);int max_num=V[0];int counter = 1;if(key==1){return max_num;}else{REP(i,V.size()){if(V[i]==max_num){continue;}else{max_num = V[i];counter++;if(counter==key){return max_num;}}}}}

signed main() {
  RAPID;
  // デフォルト変数定義
  double n=0,m=0,a=0,b=0,c=0,d=0,x=0,y=0,z=0;
  string s="",t="";
  //
  ...
  X2 = X;
  SORT(X2);
  double mid = (X2[n/2] + X2[n/2-1])/2;
  //前処理
  REP(i,n){
    if(X[i] <= mid) TEST[i] = true;
  }
  int up = X2[n/2];
  int down = X2[n/2-1];
  REP(i,n){
