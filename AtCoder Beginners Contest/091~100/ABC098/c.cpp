...
signed main() {
  RAPID;
  // デフォルト変数定義
  int n=0,m=0,a=0,b=0,c=0,d=0,x=0,y=0,z=INF;
  string s="",t="";
  //
  ...
  if(s[n-1]=='E') right.pb(1);
  else right.pb(0);
  FOR(i,1,n){
    if(s[i]=='W') left.pb(left[i-1]+1);
    else left.pb(left[i-1]);
    if(s[n-i-1]=='E') right.pb(right[i-1]+1);
    else right.pb(right[i-1]);
  }
  REV(right);
  REP(i,n){
    z = min(z, left[i]+right[i]-1);
  }
  OUT(z);
}