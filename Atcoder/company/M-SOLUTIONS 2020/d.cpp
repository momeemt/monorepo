...
signed main() {
  ...
  bool down = true;
  int gidx = 0;
  updown.pb(make_pair(A[0], -INF));
  FOR(i,1,n) {
    if (updown[gidx].first >= A[i]) {
      updown[gidx].first = A[i];
    } else {
      updown[gidx].second = A[i];
    }
    if(i < (n-1)) {
      if (A[i] != A[i+1] and updown[gidx].second != -INF) {
        updown.pb(make_pair(A[i], -INF));
        gidx++;
      }
    }
  }
  for(auto vc: updown) {
    if(vc.second == -INF) {
      ...
      int kabu = money / vc.first;
      money -= kabu * vc.first;
      money += kabu * vc.second;
    }
  }
  out(money);
}