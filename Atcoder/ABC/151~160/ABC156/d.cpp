...
int comb_mod(int n, int r, int en) {
	long long res = 1;
	for (int i=0; i<r; ++i)
		res = (res * (n - i) )%MOD;
	return ( res * en ) % MOD;
...
signed main() {
  RAPID;
  // デフォルト変数定義
  int n=0,m=0,a=0,b=0,c=0,d=0,x=1,y=0,z=0;
  string s="",t="";
  //
  ...
  factnum = fact_mod(n,MOD);
  expnum = exp_mod(factnum,MOD-2, MOD);
  int ans = x-comb_mod(n,a,expnum)-comb_mod(n,b,expnum)-1;
  if(ans >= 0) OUT(ans%MOD);
  else OUT((ans+MOD)%MOD);
}